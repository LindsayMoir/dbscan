...
def display_circles(pcs, n_comp, pca, axis_ranks, labels=None, label_rotation=0, lims=None):
    ...
    for d1, d2 in axis_ranks: 
        if d2 < n_comp:
            ...
            plt.xlabel('PC{} ({}%)'.format(d1, round(100*pca.explained_variance_ratio_[d1],1)))
            plt.ylabel('PC{} ({}%)'.format(d2, round(100*pca.explained_variance_ratio_[d2],1)))

            plt.title("Correlation Circle (PC{} and PC{})".format(d1, d2))
...
def display_factorial_planes(X_projected, n_comp, pca, axis_ranks, labels=None, alpha=1, illustrative_var=None):
    ...
    for d1,d2 in axis_ranks:
        if d2 < n_comp:
            ...
            plt.title("Projection of points (on PC{} and PC{})".format(d1, d2))
            #plt.show(block=False)
